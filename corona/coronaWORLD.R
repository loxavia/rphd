...
#rename Columns
newcols = c('country', 'Cases','newCases','Deaths','newDeaths', 'Recovered', 'activeCases','serious', 'casePerM','deathsPerM','firstCase')
length(newcols)
(today = Sys.Date())
(yesterday = today-1)
...
ctable1 <- ctable1 %>% mutate_at(vars(2:10), ~as.numeric(gsub(',' , '', .)))
...
ctable2Melt1 <- ctable2 %>% filter(Cases > 10000 | country=='India') %>% reshape2::melt(id.var=c('country','status','firstCase'))
head(ctable2Melt1)
str(ctable2Melt1)
clist2 <- ctable2Melt1 %>% distinct(country) %>% pull(country)

ctable1Melt1 <- ctable1 %>% filter(Cases > 10000 | country=='India') %>% reshape2::melt(id.var=c('country','status','firstCase'))
head(ctable1Melt1)
str(ctable1Melt1)
...
#firstCase DF----
clist1
ctable2 %>% select(country, firstCase) %>% filter(country %in% clist2)
ctable1 %>% select(country, firstCase) %>% filter(country %in% clist1)

(case2 = ctable2 %>% select(country, firstCase) %>% filter(country %in% clist2))
(case1 = ctable1 %>% select(country, firstCase) %>% filter(country %in% clist1))
names(ctable1)
...
(ctable2Melt2 <- ctable2 %>% select(-firstCase) %>% filter(Cases > 1000 | country=='India') %>% reshape2::melt(id.var=c('country','status')))
head(ctable2Melt2); head(fcaseList2)
dim(ctable2Melt2); dim(fcaseList2)
str(ctable2Melt2); str(fcaseList2)
...
(vnames = names(table(ctable2Melt2a$variable)))
(caseOrder = vnames[c(3,7,4,8,1,9,10,2, 5, 6)])
ctable2Melt2a$variable = factor(ctable2Melt2a$variable, ordered=T, level=caseOrder)
str(ctable2Melt2a)
summary(ctable2Melt2a)
ctable2Melt2a %>% group_by(country, variable)  %>% summarise(value=sum(value)) %>% dcast(country ~ variable, value.var='value') %>% arrange(desc(Cases))

#ctable1------
ctable1Melt2 <- ctable1 %>% select(-firstCase) %>% filter(Cases > 2000 | country=='India') %>% reshape2::melt(id.var=c('country','status'))
head(ctable2Melt2)
head(ctable1Melt2)
...
ctable1Melt2a$variable = factor(ctable1Melt2a$variable, ordered=T, level=caseOrder)

#function - heatmap----
gheat <- function(df, status) {
  ggplot(df, aes(x=country, y=variable, fill=value)) + geom_tile(color='black') + geom_text(aes(label=value, size=value, angle=30)) + scale_fill_gradient2(low='blue', high='red') + scale_size(range=c(3,3.5)) +  theme(axis.text.x = element_text(angle=30, size=rel(1)), legend.position = 'top', plot.title = element_text(hjust = 0.5, color = "#666666")) + labs(title=paste('gheat- :  : Date- ', status, ': Status of Corona : totalCases > 2000 and India'), subtitle=paste('First Case : WeekNos (01Jan-1st week) & in India :', fcaseIndia) , caption =caption1, x='Country', y='Cases') + guides(fill=F, size=F)
}

gheat2 = gheat(df=ctable1Melt2a, status=yesterday)
gheat2
gheat1 = gheat(df=ctable1Melt2a, status=today)
gheat1

#combine-----
ctable2M <- ctable2 %>% select(-firstCase) %>% reshape2::melt(id.var=c('country','status'))
ctable1M <- ctable1 %>% select(-firstCase) %>% reshape2::melt(id.var=c('country','status'))
...
selectCols1 = c('Cases','newCases','Deaths','newDeaths','Recovered','serious','activeCases')
table(both1$variable)
?geom
...
gbarB1a
both1
(selectCols2 = c('Cases','newCases','Deaths','newDeaths','serious','Recovered','casePerM', 'deathsPerM'))
...
#daily run this code
both1
(summary1 <- both1 %>% filter(variable != 'casePerM' & variable != 'deathsPerM' & country != 'Total:') %>% group_by(status, variable) %>% summarise(total = sum(value, na.rm=T)) %>% reshape2::dcast(status ~ variable, value.var='total')) 

(summary2 <- both1 %>% filter(variable != 'casePerM' & variable != 'deathsPerM' & country != 'Total:') %>% group_by(status, country,variable) %>% summarise(total = sum(value, na.rm=T)) %>% reshape2::dcast(status + country ~ variable, value.var='total') %>% arrange(country))
head(summary1)
head(summary2)
topCountry[-1]
countries
...
gbar2b
graphsList  = list(gCSum1, gCSum2, gTSum3, gbarBoth1a , gbarBoth1b, gbar2, gheat2, gbar1, gheat1, gLine1, gLine2, gTSbar1b,gbarIndia1A, gbarIndia1B, gbarIndia3,gTBLine1, gTBLine2, gTBArea2)
#gCSum2gbarIndia1A
