...
both1 %>% filter(variable != 'casePerM' | variable != 'deathsPerM') %>% group_by(status, variable) %>% summarise(total = sum(value, na.rm=T)) %>% reshape2::dcast(status ~ variable, value.var='total')
...
table(both1$variable)
gbarB1a <- ggplot(data=both1 %>% filter(variable %in% selectCols1 & country %in% topCountry), aes(x=variable, y=value, fill=factor(status))) + geom_bar(stat='identity', position=position_dodge2(.7)) + facet_wrap(country ~., scale='free') + geom_text(aes(label=value, y=value), angle=90,size=rel(2.5), position=position_dodge2(.7), vjust=0, hjust=0) + theme(axis.text.x = element_text(angle=90, size=rel(.8)),legend.position = 'top', plot.title = element_text(hjust = 0.5, color = "#666666")) + labs(title=paste('gbarB1a: ' ,'Today & Yesterday', ' : Status of Corona : Total, Top 10 countries : My Country(India) : Free Scaling'),caption = caption1, y='Numbers', x='Cases')
gbarB1a
both1
